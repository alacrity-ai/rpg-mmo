...
export default class AreaMapMenu extends BaseMenu {
    ...
    setupAreaMap(areaData, playerArea = 1, debug = false) {
        ...
        while (queue.length > 0) {
            ...
            if (data.north !== null && y - 1 >= 0 && !visited.has(data.north)) {
                queue.push({ area: data.north, x, y: y - 1 });
            }
            if (data.south !== null && y + 1 < this.gridSize && !visited.has(data.south)) {
                queue.push({ area: data.south, x, y: y + 1 });
            }
            if (data.east !== null && x + 1 < this.gridSize && !visited.has(data.east)) {
                queue.push({ area: data.east, x: x + 1, y });
            }
            if (data.west !== null && x - 1 >= 0 && !visited.has(data.west)) {
